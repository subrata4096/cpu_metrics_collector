...
{
        PAPI_get_event_info(i,pset);
                 cout << "Event name: " << pset->symbol  << "\n";
	if (PAPI_query_event(i) && (IS_PRESET(i)) && (i != PAPI_TOT_CYC))
	{
                

		if (PAPI_add_event(EventSet, i) == PAPI_OK)
		{
			cout << "Adding " << pset->symbol  << "\n";
		}
		else
		{
			pset++;
...
val = (long_long *)malloc(max_to_add*sizeof(long_long));
